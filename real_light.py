...
class Light_ChannelFromer(nn.Module):
    def __init__(self, input_dim =36, embed_dim = 216, out_dim=72, num_heads=2, device='cuda'):
        super().__init__()
        self.conv = nn.Conv2d(in_channels=2, out_channels=16, kernel_size=(1, 3), padding=(0, 1))
        self.init_embeding = nn.Linear(input_dim, out_dim)
        ############################################
        # self.encoder = Encoder(out_dim, embed_dim, num_heads, device='cuda')
        ##or
        self.encoder1 = Encoder(out_dim, embed_dim, num_heads, device='cuda')
        self.encoder2 = Encoder(out_dim, embed_dim, num_heads, device='cuda')
        ############################################
        self.decoder = decoder(out_dim, out_dim, device='cuda')
    ...
    def forward(self, input):
        #B, 2, R/I, 36
        #B, 14, R/I, 72
        input = torch.view_as_real(input).transpose(-1, -2)
        x = self.conv(input)
        x = self.init_embeding(x)
        ############################################
        # x = self.encoder(x)
        ##or
        x = self.encoder1(x)
        ...
        final_out = torch.view_as_complex(out)
        return final_out
...
class ffn(nn.Module):
    def __init__(self, input_dim):
        ...
        # self.fc2 = nn.Linear(input_dim*5, input_dim)
...
class decoder(nn.Module):
    def __init__(self, input_dim = 72, out_dim=72, device='cuda'):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels=16, out_channels=14, kernel_size=(1, 3), padding=(0, 1))
        self.conv2 = nn.Conv2d(in_channels=14, out_channels=14, kernel_size=(1, 3), padding=(0, 1))
...
if __name__=='__main__':
    model = Light_ChannelFromer().cuda()
    summary(model, input_size=(2, 2, 36), dtypes=torch.float)
    #input = 256, 2, 2, 36
    #output: 256, 14, 2, 72
    x = torch.tensor(torch.rand((256, 2, 2, 36), dtype=torch.float32)).cuda() 
